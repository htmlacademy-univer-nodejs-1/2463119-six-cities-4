...
export class DefaultRentOfferService implements RentOfferService {
  ...
  public async find(
    limit: number = Constants.RENT_OFFERS_LIMIT
  ): Promise<types.DocumentType<RentOfferEntity>[]> {
    this.logger.info('Try find rent offers');

    const searchResult = await this.rentOfferModel.aggregate([
      {
        $lookup: {
          from: 'comments',
          let: { rentOfferId: '$_id' },
  ...
  public async patch(
    ...
    const result = await this.rentOfferModel.findByIdAndUpdate(
      rentOfferId,
      dto,
      { new: true }
    );
  ...
  public async findPremiumByCity(
    ...
    const result = await this.rentOfferModel.find(
      { city: city, isPremium: true },
      {},
      { limit: Constants.RENT_OFFERS_PREMIUM_LIMIT }
    );
  ...
  public async calculateRating(
    ...
    await this.rentOfferModel.findByIdAndUpdate(
      offerId,
